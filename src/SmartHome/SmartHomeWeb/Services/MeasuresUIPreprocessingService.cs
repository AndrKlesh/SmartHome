namespace SmartHomeWeb.Services
{
  ...
  public class MeasuresUIPreprocessingService(MeasuresStorageService _measuresStorageService)
  {
    public IEnumerable<string> EnumerateAllMeasures() 
    {
      return _measuresStorageService.GetAllMeasures().Select(m => 
      {
        return $"<p>{_names[m.Topic]}</p><p>{m.Value} {_units[m.Topic]}</p>";
      });
    }
